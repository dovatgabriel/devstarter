...
async function runScript(dir, script) {
    ...
    try {
        ...
        console.error(chalk.red("Script failed."), e instanceof Error ? e.message : e);
        process.exitCode = 1;
    }
}
async function chooseScript(scripts) {
...
async function browseForProject(startDir) {
...
async function main() {
    ...
    console.log(chalk.bold(`devstarter`) +
    ...
    if (!pkg) {
        console.log(chalk.yellow("No package.json found here."), "Nothing to run. Returning to your original directory.");
